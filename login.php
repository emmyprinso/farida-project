...
if(empty($usernameErr) and empty($passwordErr)){
	$checking = checkForUser($username, $password);
	...
	if($checking == "exist"){
		...
		$concern = "user name or password is incorrect";
	}
}
	...
	if(!$conn){
		...
		if(mysqli_num_rows($result) > 0){
			while($row = mysqli_fetch_assoc($result)){
					if(strtolower($row["username"]) == strtolower($user) and strtolower($row["password"]) == strtolower($mail)){
						$check = "exist";
						break;
...
<div style="width:40%;height:100%;float:left;background-image:url(pcImage.jpg);background-repeat:repeat-y;">
...
<form style="float:left;padding-left:20%;padding-top:10%;" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
...
<input type="submit" name="submit" value="Submit" class="formElement" style="margin-left:30%;"> 
</form>
<?php
